...
#The two lines added by Beau Graham:
loggedInUser=$(/bin/ls -l /dev/console | /usr/bin/awk '{ print $3 }')
echo "${loggedInUser}"
#
# Rename computer
#
SAFE_LOCAL_HOST_NAME=`echo "${loggedInUser}" | awk -F. '{ print $1 }'`
ATTEMPTS=0
MAX_ATTEMPTS=12
while [ "_${SUCCESS}" = "_" ]
do
...
if [ "_${SUCCESS}" = "_YES" ]
then
  echo "Setting computer name to \"${loggedInUser}\"..." 2>&1
  /usr/sbin/scutil --set ComputerName  "${loggedInUser}"
